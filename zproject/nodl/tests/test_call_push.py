...
class DispatchCallPushFanOutTest(ZulipTestCase):
    ...
    @patch("zproject.nodl.services.call_push_service.send_voip_push_ios")
    @patch("zproject.nodl.services.call_push_service.send_fcm_call_data")
    def test_initiating_device_excluded_from_ring_fan_out(
        self, mock_fcm: MagicMock, mock_ios: MagicMock
    ) -> None:
        """The device a call was initiated from must not ring for that call
        — but the callee's other devices still do (founder repro:
        multi-login self-ring)."""
        caller = self.example_user("hamlet")
        # The initiating phone registered its token under both accounts.
        DeviceVoipToken.objects.create(
            user=caller,
            platform="ios",
        ...
        dispatch_call_push(
            self.profile_a.id, self.call_id, self.room_name, "Caller", "",
            caller_id=caller.id, caller_device_id="shared-phone",
        )
    ...
    @patch("zproject.nodl.services.call_push_service.send_voip_push_ios")
    @patch("zproject.nodl.services.call_push_service.send_fcm_call_data")
    def test_other_shared_device_still_rings(
        self, mock_fcm: MagicMock, mock_ios: MagicMock
    ) -> None:
        """Founder repro round 2: BOTH accounts signed in on BOTH devices.
        Excluding every caller-registered token silenced the second device
        too — no phone rang and the call went straight to missed. Only the
        initiating device may be excluded."""
        caller = self.example_user("hamlet")
        for user in (caller, self.profile_a):
            DeviceVoipToken.objects.create(
                ...
                platform="ios",
                device_id="device-1",
                voip_token="apns-device-1",
            )
            DeviceVoipToken.objects.create(
                user=user,
                platform="ios",
                device_id="device-2",
                voip_token="apns-device-2",
            )

        dispatch_call_push(
            self.profile_a.id, self.call_id, self.room_name, "Caller", "",
            caller_id=caller.id, caller_device_id="device-1",
        )

        mock_ios.assert_called_once_with(
            "apns-device-2", self.call_id, self.room_name, "Caller", "",
            **default_recipient_kwargs(self.profile_a.id, str(self.ws_a)),
        )

    @patch("zproject.nodl.services.call_push_service.send_voip_push_ios")
    @patch("zproject.nodl.services.call_push_service.send_fcm_call_data")
    def test_initiating_device_sibling_registration_excluded(
        self, mock_fcm: MagicMock, mock_ios: MagicMock
    ) -> None:
        """The device lookup spans the caller HUMAN: a token the initiating
        device registered under the caller's other-workspace profile is
        excluded too."""
        ...
        # Initiating device: token registered under the caller's sibling
        # profile and under the callee.
        DeviceVoipToken.objects.create(
            user=caller_b,
            platform="android",
        ...
        dispatch_call_push(
            self.profile_a.id, self.call_id, self.room_name, "Caller", "",
            caller_id=caller_a.id, caller_device_id="shared-pixel",
        )
    ...
    @patch("zproject.nodl.services.call_push_service.send_voip_push_ios")
    @patch("zproject.nodl.services.call_push_service.send_fcm_call_data")
    def test_device_exclusion_covers_push_device_token_fallback(
        self, mock_fcm: MagicMock, mock_ios: MagicMock
    ) -> None:
        """The Android PushDeviceToken fallback must not resurrect a token
        excluded as the initiating device's (same physical device = same
        FCM token value)."""
        from zerver.models import PushDeviceToken
        ...
        dispatch_call_push(
            self.profile_a.id, self.call_id, self.room_name, "Caller", "",
            caller_id=caller.id, caller_device_id="shared-pixel",
        )
    ...
    @patch("zproject.nodl.services.call_push_service.send_voip_push_ios")
    @patch("zproject.nodl.services.call_push_service.send_fcm_call_data")
    def test_without_caller_device_id_shared_token_still_rings(
        self, mock_fcm: MagicMock, mock_ios: MagicMock
    ) -> None:
        """Old clients don't send a device id — no exclusion applies (the
        self-ring echo is then handled client-side by newer builds)."""
        caller = self.example_user("hamlet")
        for user in (caller, self.profile_a):
            DeviceVoipToken.objects.create(
                user=user,
...
class DispatchCallPushAsyncTest(TestCase):
    ...
    def test_async_dispatch_spawns_thread(self, mock_dispatch: MagicMock) -> None:
        ...
        mock_dispatch.assert_called_once_with(
            self.callee.id, call_id, room_name, "Caller", "",
            caller_id=None, caller_device_id=None,
        )
